...
void binary_join(GHashRelContainer* inner, GHashRelContainer* outer,
    ...
    checkCuda(cudaMalloc((void**) &join_res_raw_data, total_result_rows * reorder_array_size * sizeof(column_type)));
    get_join_result<<<grid_size, block_size>>>(inner_device, outer_device, outer->index_column_size, reorder_array_device, reorder_array_size,
                                               join_res_raw_data, result_counts_array, result_counts_offset, direction);
    checkCuda(cudaDeviceSynchronize());
    timer.stop_timer();
    ...
    load_relation(output_newt, reorder_array_size, join_res_raw_data, total_result_rows, 1, 0.6,
...
void transitive_closure(GHashRelContainer* edge_2__2_1, GHashRelContainer* path_2__1_2,
    ...
    while(true) {
        ...
        binary_join(edge_2__2_1, path_delta, path_newt, reorder_array, 2, JoinDirection::LEFT,
                    grid_size, block_size,
        ...
        // drop the index of delta once merged, because it won't be used in next iter
        // when migrate more general case, this operation need to be put off to end of all
        // RA operation in current iteration
        ...
        // TODO: optimize here, this can be directly used as tuples in next delta
